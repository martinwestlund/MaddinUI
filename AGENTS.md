...
- Cell_Ascension profile data is intentionally bundled from live SavedVariables. For profile update requests, read `/mnt/c/Program Files (x86)/Ascension Launcher/resources/client/WTF/Account/MADDINWINS/SavedVariables/Cell_Ascension.lua`, extract the `CellDB` table into `Profiles/Data/Cell_Ascension.lua` as `MaddinUI.profileData.Cell_Ascension`, and keep Blizzard party/raid/raid manager hiding enabled for Cell. When updating ElvUI profiles, keep ElvUI nameplates, party frames, and all raid frames disabled so KuiNameplates and Cell_Ascension own those UI areas.
...
- Document whether a profile/snippet targets:
  ...
  - Details/KuiNameplates/Cell_Ascension versions bundled or known to work on Ascension
